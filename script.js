...
function submarineAttack() {
  const damage = Math.floor(Math.random() * 20) + 10; // 
  carrier.health -= damage;

  alert(`A submarine attacks! You take ${damage} damage!`);

  if (carrier.health <= 0) {
    alert("Your fleet has been destroyed!");
    // optional: reset game or end it
  } else {
...
function triggerEvent() {
  ...
  if (chance < 0.8) { 
    const events = [
      "You spot enemy aircraft overhead!",
      "A storm rolls in, reducing visibility.",
      "Your scouts report unusual activity nearby."
    ];
