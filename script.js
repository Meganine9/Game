...
function repairCarrier() {
  ...
  if (repairCooldown > 0) {
  addLog(`❌ Repairs on cooldown (${repairCooldown} turns left)`);
  return;
}
...
function checkObjectivesComplete() {
  if (
    !missionCompleteAnnounced &&
    config.checkWin(objectives)
  ) {
    missionCompleteAnnounced = true;

    addLog(
      "✅ Objectives complete! Move to end to finish the level."
    );
  }
}
  ...
  function move(cmd) {
    ...
    addLog(`➡️ Moved to ${config.rooms[currentRoom].name}`);
    updateStats();
  }
...
const level1 = {
  ...
  commands: {
    scout: ({ objectives, currentRoom, addLog }) => {
      ...
      if(objectives.scouted == true){
        addLog("scouting already done");
        return;
      }
      ...
      if (Math.random() < 0.7) {
        ...
        addLog("❌ Failed scout");
        carrier.divebombers = Math.max(0, carrier.divebombers - 1);
        updateStats();
      }
      checkObjectivesComplete();
    },
...
const level2 = {
  ...
  commands: {
    support: ({ objectives, currentRoom, addLog }) => {
      ...
      if(objectives.supported == true){
        addLog("troops already suported");
        return;
      }
