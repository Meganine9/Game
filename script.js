...
function instructionsScene(root) {
  ...
  text.textContent = "In this game you command an aircraft carrier in 1 of 3 diffrent levels, using text input to move, attack, and recover/heal";
...
function repairCarrier() {
  ...
  addLog(`🛠️ Repairs complete (+${heal} HP)`);
  repairCooldown++
  updateStats();
}
  ...
  return;
    ...
    function handlePlane(type, count, base, aaMod) {
      for (let i = 0; i < count; i++) {
        let idx = Math.random() < 0.6 ? target : getSmartTarget();
        if (idx === null) break;

        let enemy = currentEnemies[idx];

        if (Math.random() < enemy.aa * aaMod) {
          if (carrier[type] <= 0) continue;
          carrier[type]--;
...
const level1 = {
  intro: "Scout Wake or attack fleet?...commands: to move enter the name of where you want to go either wake or enemy, to attack be in the enemy room and enter attack index of enemy you want to focas on # of fighters # of dive bombers and # of torpedo bombers ex: attack 0 10 5 5, to scout move to the wake room and enter scout, good luck!",
  startRoom: "start",
